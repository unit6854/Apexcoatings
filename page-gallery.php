...
// Exclude WP-generated thumbnail variants, then sort newest file first (by filesystem mtime)
$image_files = array_values(array_filter($all_webp, function($f) {
    $base = basename($f);
    return !preg_match('/-\d+x\d+\.webp$/', $base) && !preg_match('/-scaled\.webp$/', $base);
}));
...
<div style="background:var(--apex-black);padding:140px 0 60px;">
    <div class="container">
        ...
        <p style="color:rgba(255,255,255,0.55);max-width:560px;line-height:1.7;margin-top:16px;">Real jobs. Real craftsmanship. Every piece shown was built by hand — right here in our shop.</p>
    </div>
</div>
...
<section style="background:#111;padding:48px 0 80px;">
    <div class="container">
        <div id="gallery-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:16px;">
            <?php foreach ($by_cat as $cat => $items): ?>
                ...
                <?php
                    ...
                    $num      = preg_replace('/[^0-9]/', '', substr($filename, -6));
                ?>
        ...
        <div style="text-align:center;padding:80px 40px;color:rgba(255,255,255,0.4);">
            ...
            <a href="<?php echo home_url('/contact'); ?>" class="btn btn-primary" style="margin-top:24px;">Contact Us</a>
        </div>
        <?php endif; ?>
    </div>
</section>
...
<div id="apex-lightbox" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:9999;align-items:center;justify-content:center;" onclick="closeLightbox()">
    ...
    <img id="apex-lightbox-img" src="" alt="" style="max-width:92vw;max-height:88vh;object-fit:contain;border-radius:4px;box-shadow:0 8px 48px rgba(0,0,0,0.6);display:none;">
