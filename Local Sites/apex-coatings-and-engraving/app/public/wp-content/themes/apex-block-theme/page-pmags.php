...
<div class="pmag-hero">
    <div class="container">
        <span class="eyebrow" style="display:block;margin-bottom:10px;"><?php echo esc_html( apex_opt( 'pmags_page_eyebrow', 'Genuine Magpul PMAGs' ) ); ?></span>
        <h1 class="section-title" style="color:var(--apex-white);margin-bottom:16px;"><?php echo esc_html( apex_opt( 'pmags_page_h1', 'Custom PMAG Engraving' ) ); ?></h1>
        <p class="pmag-hero-desc"><?php echo esc_html( apex_opt( 'pmags_page_desc', 'We only use genuine Magpul PMAGs and laser engrave custom designs and art directly into them. Each magazine is individually engraved — no stickers, no paint, no fading. Built to last as long as the mag itself.' ) ); ?></p>
        <div class="pmag-badges">
            <span class="pmag-badge"><?php echo esc_html( apex_opt( 'pmags_page_badge_1', '✓ Genuine Magpul PMAGs Only' ) ); ?></span>
            <span class="pmag-badge"><?php echo esc_html( apex_opt( 'pmags_page_badge_2', '✓ Laser Engraved' ) ); ?></span>
            <span class="pmag-badge"><?php echo esc_html( apex_opt( 'pmags_page_badge_3', '✓ Custom Artwork Available' ) ); ?></span>
            <span class="pmag-badge"><?php echo esc_html( apex_opt( 'pmags_page_badge_4', '✓ Bulk discounts available' ) ); ?></span>
        </div>
    </div>
</div>
    ...
    <div class="container">
        <div class="pmag-info-grid">
            <div class="pmag-info-item">
                ...
                <div>
                    <strong><?php echo esc_html( apex_opt( 'pmags_info_1_title', 'Genuine Magpul Only' ) ); ?></strong>
                    <span><?php echo esc_html( apex_opt( 'pmags_info_1_desc', 'No off-brand substitutes' ) ); ?></span>
                </div>
            </div>
            <div class="pmag-info-item">
                ...
                <div>
                    <strong><?php echo esc_html( apex_opt( 'pmags_info_2_title', 'Turnaround' ) ); ?></strong>
                    <span><?php echo esc_html( apex_opt( 'pmags_info_2_desc', '1–3 business days' ) ); ?></span>
                </div>
            </div>
            <div class="pmag-info-item">
                ...
                <div>
                    <strong><?php echo esc_html( apex_opt( 'pmags_info_3_title', 'Pricing' ) ); ?></strong>
                    <span><?php echo esc_html( apex_opt( 'pmags_info_3_desc', '$35 includes magazine and engraving on both sides' ) ); ?></span>
                </div>
            </div>
            <div class="pmag-info-item">
                ...
                <div>
                    <strong><?php echo esc_html( apex_opt( 'pmags_info_4_title', 'Custom Designs' ) ); ?></strong>
                    <span><?php echo esc_html( apex_opt( 'pmags_info_4_desc', "Don't see what you want? Ask us" ) ); ?></span>
                </div>
            </div>
        </div>
    </div>
...
<section class="pmag-section">
    <div class="container">
        <div class="pmag-section-header">
            <h2 class="section-title" style="color:var(--apex-white);"><?php echo esc_html( apex_opt( 'pmags_grid_heading', 'Choose Your Design' ) ); ?></h2>
            <p style="color:rgba(255,255,255,0.55);margin-top:8px;"><?php echo esc_html( apex_opt( 'pmags_grid_subtext', 'Select a design below and add it to your cart. After we clear your shipping address, you will receive an invoice by email to complete your purchase.' ) ); ?></p>
        </div>
            ...
            <div class="pmag-card reveal" data-design="<?php echo esc_attr($cart_id); ?>">
                ...
                <div class="pmag-card-body">
                    <div class="pmag-card-top">
                        ...
                        <span class="pmag-price-tag"><?php echo esc_html( apex_opt( 'pmags_card_price', 'Only $35' ) ); ?></span>
                    </div>
                    <p class="pmag-card-desc"><?php echo esc_html( apex_opt( 'pmags_card_subdesc', 'Genuine Magpul PMAG · Laser Engraved · Custom artwork' ) ); ?></p>
        ...
        <div class="pmag-custom-cta">
            <div class="pmag-custom-inner">
                <div>
                    <h3 style="color:var(--apex-white);font-family:'Barlow Condensed',sans-serif;font-size:1.6rem;font-weight:800;text-transform:uppercase;letter-spacing:0.04em;margin:0 0 8px;"><?php echo esc_html( apex_opt( 'pmags_cta_heading', "Don't See What You Want?" ) ); ?></h3>
                    <p style="color:rgba(255,255,255,0.6);margin:0;"><?php echo esc_html( apex_opt( 'pmags_cta_desc', "Have something specific in mind? We engrave custom artwork, logos, text, and anything else you can dream up. Bring your idea — we'll bring it to life on your mag." ) ); ?></p>
                </div>
                <a href="<?php echo esc_url( home_url('/contact') ); ?>" class="btn btn-outline" style="white-space:nowrap;flex-shrink:0;">Request Custom Design</a>
            </div>
        </div>
    </div>
</section>
...
<div id="pmag-lightbox" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.95);z-index:9999;align-items:center;justify-content:center;flex-direction:column;" onclick="closePmagLightbox()">
    ...
    <img id="pmag-lightbox-img" src="" alt="" data-no-optimize="1" style="max-width:92vw;max-height:85vh;width:auto;height:auto;object-fit:contain;border-radius:6px;box-shadow:0 8px 48px rgba(0,0,0,0.6);display:block;" onclick="event.stopPropagation()">
...
<script>
// Fix: body has CSS transform which breaks position:fixed — move lightbox to <html>
(function(){ var _lb = document.getElementById('pmag-lightbox'); if (_lb) document.documentElement.appendChild(_lb); })();
...
function apexPmagAddToCart(id, name) {
    ...
    var imgSrc = '';
    if (btn) {
        var card = btn.closest('.pmag-card');
        if (card) {
            var img = card.querySelector('.pmag-card-img');
            if (img) imgSrc = img.src;
        }
    }

    window.ApexCart.addItem({ id: id, name: name, price: 35, desc: 'Custom laser-engraved PMAG design', image: imgSrc, quantity: 1 });
...
<!-- Internal Silo Link — 1911 Grips -->
<div style="background:var(--apex-black);padding:32px 0;border-top:1px solid rgba(245,131,31,0.12);">
    <div class="container" style="text-align:center;">
        <p style="color:rgba(255,255,255,0.6);font-size:0.95rem;margin:0;">
            Also available: <a href="<?php echo esc_url( home_url('/1911-grips') ); ?>" style="color:var(--apex-orange);font-weight:600;">custom 1911 grip panels</a> starting at $90 &mdash; American-made, finished in-house.
        </p>
    </div>
</div>

<!-- Magazine Shipping Restriction Disclaimer -->
